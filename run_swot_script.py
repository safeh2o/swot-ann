...
def run_swot():
    ...
    SWOT_net.set_up_model()
    SWOT_net.train_SWOT_network(directory)
    SWOT_net.calibration_performance_evaluation(report_file)
    SWOT_net.post_process_cal()
    # SWOT_net.full_performance_evaluation(directory)
    SWOT_net.set_inputs_for_table(storage_target)
    SWOT_net.import_pretrained_model(directory)
    SWOT_net.predict()
    SWOT_net.display_results()
    SWOT_net.export_results_to_csv(results_file)
    SWOT_net.generate_html_report(report_file, storage_target)
